...
#' @param dim the number of eigenvalues.
...
#' @export
#' 
BSeigenval <- function(INT, dim, sigma, tol = 0.0001){
  L1 <- (dim - 1)*sqrt(INT/100) + 1 #leading eigenvector
  Lk <- rep((dim - L1)/(dim - 1), dim-1) #other eigenvectors
  d <- c(L1, Lk) + rnorm(dim, mean = 0, sd = sigma)
